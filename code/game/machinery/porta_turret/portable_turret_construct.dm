...
/obj/machinery/porta_turret_construct
	...
	var/turrettype = /obj/machinery/porta_turret

/obj/machinery/porta_turret_construct/attackby(obj/item/I, mob/user, params)
	//this is a bit unwieldy but self-explanatory
	switch(build_step)
		if(PTURRET_UNSECURED)	//first step
			if(pturret_unsecured_stage(I, user))
				return

		if(PTURRET_BOLTED)
			if(pturret_bolted_stage(I, user))
				return

		if(PTURRET_START_INTERNAL_ARMOUR)
			if(pturret_internalarmor_stage(I, user))
				return

		if(PTURRET_INTERNAL_ARMOUR_ON)
			if(pturret_afterarmor_stage(I, user))
				return

		if(PTURRET_GUN_EQUIPPED)
			if(pturret_aftergun_stage(I, user))
				return

		if(PTURRET_SENSORS_ON)
			if(pturret_afterproxi_stage(I, user))
				return

		if(PTURRET_CLOSED)
			if(pturret_closed_stage(I, user))
				return

		if(PTURRET_START_EXTERNAL_ARMOUR)
			if(pturret_last_stage(I, user))
				return
...
/obj/machinery/porta_turret_construct/update_icon()
	if(build_step >= PTURRET_START_INTERNAL_ARMOUR)
...
/obj/machinery/porta_turret_construct/proc/pturret_unsecured_stage(obj/item/I, mob/user)
...
/obj/machinery/porta_turret_construct/proc/pturret_bolted_stage(obj/item/I, mob/user)
...
/obj/machinery/porta_turret_construct/proc/pturret_internalarmor_stage(obj/item/I, mob/user)
...
/obj/machinery/porta_turret_construct/proc/pturret_afterarmor_stage(obj/item/I, mob/user)
...
/obj/machinery/porta_turret_construct/proc/pturret_aftergun_stage(obj/item/I, mob/user)
	if(isprox(I))
		if(!user.drop_item())
			return
...
/obj/machinery/porta_turret_construct/proc/pturret_afterproxi_stage(obj/item/I, mob/user)
...
/obj/machinery/porta_turret_construct/proc/pturret_closed_stage(obj/item/I, mob/user)
	if(istype(I, /obj/item/stack/sheet/metal))
		var/obj/item/stack/sheet/metal/M = I
		if(M.use(2))
...
/obj/machinery/porta_turret_construct/proc/pturret_last_stage(obj/item/I, mob/user)
	if(istype(I, /obj/item/weapon/weldingtool))
		...
		if(do_after(user, 30/I.toolspeed, target = src))
			...
			var/obj/machinery/porta_turret/turret = new turrettype(loc)
