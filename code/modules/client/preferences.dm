...
/datum/preferences/proc/SetChoices(mob/user, limit = 17, list/splitJobs = list("Chief Engineer"), widthPerColumn = 295, height = 620)
	...
	for(var/datum/job/job in SSjob.occupations)
		...
		if(!job.player_old_enough(user.client))
			...
			continue
		if((job_marines_low & SQUADMA) && (rank != "Squad Marine") && !jobban_isbanned(user, "Squad Marine"))
		...
		if(rank == "Squad Marine")//marines are special
...
/datum/preferences/proc/SetJobPreferenceLevel(datum/job/job, level)
	...
	if (job.department_flag == COMMAND)
		...
		return 1
	else if (job.department_flag == ENGI)
		...
		return 1
	else if (job.department_flag == MARINES)
...
/datum/preferences/proc/UpdateJobPreference(mob/user, role, desiredLvl)
	...
	if(role == "Squad Marine")
		if(job_marines_low & job.flag)
			job_marines_low &= ~job.flag
		else
			job_marines_low |= job.flag
		SetChoices(user)
